...
GENERIC_LOW_SIGNAL_TERMS = {
    ...
    'consciencia', 'sujeito', 'reflexividade'
}

DEFAULT_HINTS = [
    'linguagem', 'símbolo', 'simbolo', 'sentido', 'significação', 'significacao', 'referência', 'referencia',
    'mediação', 'mediacao', 'representação', 'representacao', 'apreensão', 'apreensao',
    'consciência', 'consciencia', 'consciência reflexiva', 'consciencia reflexiva',
    'partilha', 'transmissível', 'transmissivel', 'estabilização simbólica', 'estabilizacao simbolica',
    'sistema de símbolos', 'sistema de simbolos', 'fechamento', 'fecho interno', 'coerência interna', 'coerencia interna',
    'erro categorial', 'erro de escala', 'adequação', 'adequacao', 'verdade', 'correção', 'correcao',
    'submissão ao real', 'submissao ao real', 'linguisticismo', 'convencionalismo', 'construtivismo semântico', 'construtivismo semantico'
]

PROFILE_CONFIGS: Dict[str, Dict[str, Any]] = {
    'simbolo_unidade_operativa_mediacao': {
        'label': 'Símbolo como unidade operativa de mediação',
        'description': 'Símbolo como unidade mínima operativa pela qual a representação se fixa, se retoma e se torna transmissível.',
        'hints': [
            'símbolo', 'simbolo', 'unidade operativa', 'unidade mínima', 'unidade minima',
            'fixação simbólica', 'fixacao simbolica', 'retoma', 'retomar', 'transmissível', 'transmissivel',
            'símbolo como unidade', 'simbolo como unidade', 'signo', 'significação', 'significacao',
            'mediação simbólica', 'mediacao simbolica', 'símbolo operativo', 'simbolo operativo'
        ],
        'boost_terms': ['simbo', 'sign', 'fixac', 'transm', 'retom', 'unida'],
        'anti_hints': ['linguisticismo forte', 'erro de escala']
    },
    'linguagem_estabilizacao_partilha_transmissibilidade': {
        'label': 'Linguagem, estabilização, partilha e transmissibilidade',
        'description': 'Linguagem como sistema estabilizado de símbolos, partilhável e transmissível, sem se confundir com a apreensão imediata.',
        'hints': [
            'linguagem', 'sistema de símbolos', 'sistema de simbolos', 'partilha', 'partilhável', 'partilhavel',
            'transmissibilidade', 'transmissão', 'transmissao', 'circulação', 'circulacao', 'uso', 'referência', 'referencia',
            'conservação', 'conservacao', 'articulação', 'articulacao', 'linguagem como sistema', 'mundo comum',
            'estabilizado de símbolos', 'estabilizado de simbolos'
        ],
        'boost_terms': ['lingu', 'parti', 'transm', 'circu', 'artic', 'conser', 'refer'],
        'anti_hints': ['pré linguístico absoluto', 'pre linguistico absoluto']
    },
    'mediacao_transformacao_apreensao_representacao': {
        'label': 'Mediação, transformação da apreensão e representação',
        'description': 'Mediação como transformação operativa da apreensão em representação estabilizada e operável.',
        'hints': [
            'mediação', 'mediacao', 'transformação operativa', 'transformacao operativa', 'apreensão', 'apreensao',
            'representação', 'representacao', 'transforma a apreensão', 'transforma a apreensao',
            'estabilizada', 'operável', 'operavel', 'mediação entre', 'passagem à representação',
            'fixação do apreendido', 'fixacao do apreendido', 'enquadramento', 'operação mediadora', 'operacao mediadora'
        ],
        'boost_terms': ['media', 'transf', 'apree', 'repre', 'oper', 'enquad'],
        'anti_hints': ['mera circulação linguística', 'mera circulacao linguistica']
    },
    'consciencia_reflexiva_localizada': {
        'label': 'Consciência reflexiva localizada',
        'description': 'Consciência reflexiva como capacidade localizada que retoma e trabalha mediações na relação com o real.',
        'hints': [
            'consciência reflexiva', 'consciencia reflexiva', 'capacidade localizada', 'localizada',
            'eu', 'relação com o real', 'relacao com o real', 'retoma', 'trabalha a mediação', 'trabalha a mediacao',
            'consciência no real', 'consciencia no real', 'interioridade', 'ponto de vista', 'reflexividade',
            'ser reflexivo', 'capacidade reflexiva'
        ],
        'boost_terms': ['consc', 'refle', 'local', 'interi', 'eu', 'vista'],
        'anti_hints': ['linguagem cria consciência do nada', 'linguagem cria consciencia do nada']
    },
    'representacao_adequacao_verdade_correcao': {
        'label': 'Representação, adequação, verdade e correção',
        'description': 'Prolongamento epistemológico: representação ajustada ou desajustada, verdade sob critério e correção por reinscrição no real.',
        'hints': [
            'adequação', 'adequacao', 'desadequação', 'desadequacao', 'verdade', 'erro', 'correção', 'correcao',
            'critério', 'criterio', 'submissão ao real', 'submissao ao real', 'reinscrição no real', 'reinscricao no real',
            'representação ajustada', 'representacao ajustada', 'ajustamento ao real', 'desajustamento',
            'critério operativo', 'criterio operativo', 'fecho interno', 'fechamento', 'coerência interna', 'coerencia interna'
        ],
        'boost_terms': ['adequa', 'verda', 'erro', 'corre', 'crite', 'reins', 'coere'],
        'anti_hints': ['sentido puro sem real', 'sentido puro sem real']
    },
    'linguisticismo_construtivismo_sentido': {
        'label': 'Linguisticismo, construtivismo e sentido',
        'description': 'Perfil opcional para medir se o léxico de sentido/construção/language-first domina artificialmente o dossier.',
        'hints': [
            'sentido', 'construção de sentido', 'construcao de sentido', 'linguisticismo', 'convencionalismo',
            'construtivismo', 'sentido como construção', 'sentido como construcao', 'o discurso cria',
            'a linguagem cria', 'mundo comum', 'constitui o real', 'constituição linguística', 'constituicao linguistica',
            'anti linguisticismo', 'pré linguístico', 'pre linguistico'
        ],
        'boost_terms': ['sentid', 'constr', 'lingui', 'conven', 'discur', 'consti'],
        'anti_hints': ['submissão ao real', 'submissao ao real']
    },
}
...
def build_overlap_signals(sample_global: List[Dict[str, Any]]) -> Dict[str, Any]:
    counts = Counter(item.get('best_profile') for item in sample_global if item.get('best_profile'))
    total = max(1, len(sample_global))
    corridor_semantic = (
        counts.get('simbolo_unidade_operativa_mediacao', 0) +
        counts.get('linguagem_estabilizacao_partilha_transmissibilidade', 0)
    ) / total
    corridor_mediational = (
        counts.get('mediacao_transformacao_apreensao_representacao', 0) +
        counts.get('consciencia_reflexiva_localizada', 0)
    ) / total
    cf07_like = (
        counts.get('mediacao_transformacao_apreensao_representacao', 0) +
        counts.get('representacao_adequacao_verdade_correcao', 0)
    ) / total
    p27_p34_pull = (
        counts.get('simbolo_unidade_operativa_mediacao', 0) +
        counts.get('linguagem_estabilizacao_partilha_transmissibilidade', 0) +
    ...
    linguisticism_pull = counts.get('linguisticismo_construtivismo_sentido', 0) / total

    notes: List[str] = []
    if corridor_semantic >= 0.40:
        notes.append('Há tração relevante para o eixo semântico-linguístico estrito: símbolo + linguagem.')
    if corridor_mediational >= 0.40:
        notes.append('Há tração relevante para o eixo mediacional: transformação da apreensão em representação + consciência reflexiva localizada.')
    if cf07_like >= 0.35:
        notes.append('Há tração relevante para a ponta CF07-like: mediação/representação + adequação/verdade/correção.')
    if p27_p34_pull >= 0.65:
        notes.append('O sample aproxima-se fortemente do corredor P27–P34: símbolo → linguagem → mediação → consciência reflexiva → representação → adequação/correção.')
    if linguisticism_pull >= 0.30:
        notes.append('O léxico de sentido/construção/language-first tem peso não negligenciável no sample.')
    if not notes:
        notes.append('Os sinais de sobreposição arquitetónica existem, mas não aparecem de forma concentrada no sample selecionado.')

    return {
        'semantic_linguistic_share': round(corridor_semantic, 4),
        'mediational_share': round(corridor_mediational, 4),
        'cf07_like_share': round(cf07_like, 4),
        'corridor_p27_p34_share': round(p27_p34_pull, 4),
        'linguisticism_share': round(linguisticism_pull, 4),
        'notes': notes,
    }
...
def build_alignment_diagnosis(
    ...
    overlap_signals = build_overlap_signals(sample_global)
...
def build_markdown(
    ...
    lines: List[str] = []
    lines.append(f'# {confronto_id} — fragmentos relevantes para o dossier (CF05 v1)')
    lines.append('')
    ...
    lines.append('```')
    lines.append('')
    lines.append('### Sinais de sobreposição arquitetónica')
    lines.append('')
    lines.append('```json')
    lines.append(json.dumps(alignment.get('overlap_signals', {}), ensure_ascii=False, indent=2))
    lines.append('```')
    lines.append('')
...
def build_report(
    ...
    lines: List[str] = []
    lines.append(f'RELATÓRIO — {confronto_id} (CF05 v1)')
    lines.append('')
    ...
    lines.append('')
    lines.append('SOBREPOSIÇÃO ARQUITETÓNICA')
    lines.append('')
    lines.append(json.dumps(alignment.get('overlap_signals', {}), ensure_ascii=False, indent=2))
    lines.append('')
    lines.append('PERFIS')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seleciona fragmentos relevantes para o dossier CF05, com score global robusto, perfis paralelos, estimação do perfil declarado e sinais de sobreposição arquitetónica.'
    )
