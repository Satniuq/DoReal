...
Objetivo:
- Ler a adjudicação filosófica em adjudicacao_confrontos_filosoficos_v1.json
- Produzir uma versão redacionalmente restringida
- Preservar subestruturas como objetos/listas, sem as achatar em strings
- Limitar amplitude temática mantendo rastreabilidade estrutural útil
...
- 16_validacao_integral/02_outputs/relatorio_restricao_adjudicacao_confrontos_filosoficos_v2.txt
"""
...
def unique_preserve_order_strings(items: List[str]) -> List[str]:
    seen = set()
    out: List[str] = []
    for x in items:
        s = str(x).strip()
...
def to_text_list(value: Any) -> List[str]:
    ...
    for item in ensure_list(value):
        ...
        if s:
            out.append(s)
    return unique_preserve_order_strings(out)


def dedupe_dicts_by_keys(items: List[Dict[str, Any]], keys: List[str]) -> List[Dict[str, Any]]:
    seen = set()
    out: List[Dict[str, Any]] = []
    for item in items:
        if not isinstance(item, dict):
            continue
        identity = tuple(item.get(k) for k in keys)
...
def build_paths(project_root: Path) -> Dict[str, Path]:
    ...
    return {
        ...
        "output_json": dados / "adjudicacao_confrontos_filosoficos_restrita_v2.json",
        "output_report": outputs / "relatorio_restricao_adjudicacao_confrontos_filosoficos_v2.txt",
    }
...
METASTRUCTURAL_HINTS = {
    ...
    "passagem entre regimes", "inteligibilidade do sistema", "metaestrutural",
    "metaarquitetonico", "sistema"
}
...
def derive_focus_tags(entry: Dict[str, Any]) -> List[str]:
    ...
    return unique_preserve_order_strings(tags)


def pick_core_strings(problem_id: str, title: str, items: List[str], max_items: int) -> List[str]:
    ...
    if problem_id == "CF01" or "ser" in title_low or "real" in title_low:
        ...
        for x in normalized:
            ...
            if any(h in xl for h in ONTOLOGY_CORE_HINTS):
                ...
                secondary.append(x)
        return truncate_list(unique_preserve_order_strings(preferred + secondary), max_items)
    ...
    return truncate_list(unique_preserve_order_strings(normalized), max_items)


def restrict_objecoes_structured(value: Any, problem_id: str, title: str, max_items: int) -> List[Dict[str, Any]]:
    ...
    for raw in items:
        if not isinstance(raw, dict):
            continue
        objecao = first_non_empty_string(raw.get("objecao"))
        resposta_curta = first_non_empty_string(raw.get("resposta_curta"))
        estrategia = first_non_empty_string(raw.get("estrategia"))
        ordem = raw.get("ordem")

        item = {
            "ordem": ordem if isinstance(ordem, int) else len(out) + 1,
            "objecao": objecao,
            "resposta_curta": resposta_curta,
            "estrategia": estrategia,
        }
        if objecao or resposta_curta or estrategia:
            out.append(item)

    out = dedupe_dicts_by_keys(out, ["objecao", "estrategia"])
    if out:
        textos = [x["objecao"] for x in out]
        escolhidos = pick_core_strings(problem_id, title, textos, max_items)
        filtrados = [x for x in out if x["objecao"] in escolhidos]
        filtrados = truncate_list(filtrados, max_items)
        for i, item in enumerate(filtrados, start=1):
            item["ordem"] = i
        return filtrados
...
def restrict_proposicoes_structured(value: Any, max_items: int) -> List[Dict[str, Any]]:
    items = ensure_list(value)
    out: List[Dict[str, Any]] = []
    ...
    for raw in items:
        if not isinstance(raw, dict):
            continue
        item = {
            "proposicao_id": first_non_empty_string(raw.get("proposicao_id")),
            "texto_curto": first_non_empty_string(raw.get("texto_curto")),
            "bloco_id": first_non_empty_string(raw.get("bloco_id")),
            "ordem_global": raw.get("ordem_global"),
            "funcao_no_mapa": first_non_empty_string(raw.get("funcao_no_mapa")),
        }
        if item["proposicao_id"]:
            out.append(item)

    out = dedupe_dicts_by_keys(out, ["proposicao_id"])
    out = sorted(out, key=lambda x: (x.get("ordem_global") is None, x.get("ordem_global", 999999)))
    return truncate_list(out, max_items)


def restrict_pontes_structured(value: Any, max_items: int) -> List[Dict[str, Any]]:
    items = ensure_list(value)
    out: List[Dict[str, Any]] = []
    for raw in items:
        if not isinstance(raw, dict):
            continue
        item = {
            "ponte_id": first_non_empty_string(raw.get("ponte_id")),
            "nivel_origem": first_non_empty_string(raw.get("nivel_origem")),
            "nivel_destino": first_non_empty_string(raw.get("nivel_destino")),
            "tipo_ponte": first_non_empty_string(raw.get("tipo_ponte")),
            "grau_risco": first_non_empty_string(raw.get("grau_risco")),
            "problema_da_transicao": first_non_empty_string(raw.get("problema_da_transicao")),
            "justificacao_provisoria": first_non_empty_string(raw.get("justificacao_provisoria")),
        }
        if item["ponte_id"]:
            out.append(item)

    out = dedupe_dicts_by_keys(out, ["ponte_id"])
    out = sorted(out, key=lambda x: (x.get("grau_risco") not in {"critico", "alto"}, x.get("ponte_id", "")))
    return truncate_list(out, max_items)


def restrict_ancoragens_structured(value: Any, max_items: int) -> List[Dict[str, Any]]:
    items = ensure_list(value)
    out: List[Dict[str, Any]] = []
    for raw in items:
        if not isinstance(raw, dict):
            continue
        item = {
            "entrada_id": first_non_empty_string(raw.get("entrada_id")),
            "tema_cientifico": first_non_empty_string(raw.get("tema_cientifico")),
            "dominios_cientificos": truncate_list(unique_preserve_order_strings(to_text_list(raw.get("dominios_cientificos"))), 6),
            "tipo_dependencia_cientifica": truncate_list(unique_preserve_order_strings(to_text_list(raw.get("tipo_dependencia_cientifica"))), 4),
            "estado_atual_da_ancoragem": first_non_empty_string(raw.get("estado_atual_da_ancoragem")),
            "restricoes_ou_alertas": truncate_list(unique_preserve_order_strings(to_text_list(raw.get("restricoes_ou_alertas"))), 3),
        }
        if item["entrada_id"]:
            out.append(item)

    out = dedupe_dicts_by_keys(out, ["entrada_id"])
    return truncate_list(out, max_items)


def restrict_campos_structured(value: Any, max_items: int) -> List[Dict[str, Any]]:
    items = ensure_list(value)
    out: List[Dict[str, Any]] = []
    for raw in items:
        if not isinstance(raw, dict):
            continue
        item = {
            "campo_id": first_non_empty_string(raw.get("campo_id")),
            "nome_campo": first_non_empty_string(raw.get("nome_campo")),
            "tipo_campo": first_non_empty_string(raw.get("tipo_campo")),
            "grau_risco": first_non_empty_string(raw.get("grau_risco")),
            "propriedades_em_foco": truncate_list(unique_preserve_order_strings(to_text_list(raw.get("propriedades_em_foco"))), 5),
            "dinamicas_relevantes": truncate_list(unique_preserve_order_strings(to_text_list(raw.get("dinamicas_relevantes"))), 3),
        }
        if item["campo_id"]:
            out.append(item)

    out = dedupe_dicts_by_keys(out, ["campo_id"])
    out = sorted(out, key=lambda x: (x.get("grau_risco") not in {"critico", "alto"}, x.get("campo_id", "")))
    return truncate_list(out, max_items)
...
def restrict_single_confronto(item: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    counters = {
        ...
        "prop_art_before": 0, "prop_art_after": 0,
        "pontes_art_before": 0, "pontes_art_after": 0,
        "anc_art_before": 0, "anc_art_after": 0,
        "campos_art_before": 0, "campos_art_after": 0,
    }

    autores = to_text_list(item.get("autores_prioritarios"))
    counters["autores_before"] = len(autores)
    autores = truncate_list(unique_preserve_order_strings(autores), MAX_AUTORES)
    ...
    tradicoes = truncate_list(unique_preserve_order_strings(tradicoes), MAX_TRADICOES)
    ...
    conceitos = pick_core_strings(cf_id, title, conceitos, MAX_CONCEITOS)
    counters["conceitos_after"] = len(conceitos)

    temas = to_text_list(item.get("temas_agregados"))
    counters["temas_before"] = len(temas)
    temas = pick_core_strings(cf_id, title, temas, MAX_TEMAS)
    ...
    teses_classicas = pick_core_strings(cf_id, title, teses_classicas, MAX_TESES_CLASSICAS)
    ...
    adversarios = truncate_list(unique_preserve_order_strings(adversarios), MAX_ADVERSARIOS)
    ...
    obj_fortes = pick_core_strings(cf_id, title, obj_fortes, MAX_OBJECOES_FORTES)
    ...
    insuf = truncate_list(unique_preserve_order_strings(insuf), MAX_INSUFICIENCIAS)
    counters["insuf_after"] = len(insuf)

    linhas = to_text_list(item.get("linhas_de_tratamento"))
    counters["linhas_before"] = len(linhas)
    linhas = truncate_list(unique_preserve_order_strings(linhas), MAX_LINHAS_TRATAMENTO)
    ...
    tipo_resp = truncate_list(unique_preserve_order_strings(tipo_resp), MAX_TIPO_RESPOSTA)
    ...
    tipo_tensao = truncate_list(unique_preserve_order_strings(tipo_tensao), MAX_TIPO_TENSAO)
    ...
    veredito = truncate_list(unique_preserve_order_strings(veredito), MAX_VEREDITO)
    counters["veredito_after"] = len(veredito)

    campos_ids = to_text_list(item.get("campo_ids_relacionados"))
    counters["campos_before"] = len(campos_ids)
    campos_ids = truncate_list(unique_preserve_order_strings(campos_ids), MAX_CAMPOS)
    counters["campos_after"] = len(campos_ids)

    pontes_ids = to_text_list(item.get("ponte_ids_relacionadas"))
    counters["pontes_before"] = len(pontes_ids)
    pontes_ids = truncate_list(unique_preserve_order_strings(pontes_ids), MAX_PONTES)
    counters["pontes_after"] = len(pontes_ids)

    ancoragens_ids = to_text_list(item.get("ancoragem_ids_relacionadas"))
    counters["anc_before"] = len(ancoragens_ids)
    ancoragens_ids = truncate_list(unique_preserve_order_strings(ancoragens_ids), MAX_ANCORAGENS)
    counters["anc_after"] = len(ancoragens_ids)
    ...
    teses_sust = pick_core_strings(cf_id, title, teses_sust, MAX_TESES_SUSTENTACAO)
    ...
    dist = pick_core_strings(cf_id, title, dist, MAX_DISTINCOES_MINIMAS)
    counters["dist_after"] = len(dist)

    obj_prio = ensure_list(adj.get("objecoes_priorizadas"))
    counters["obj_prio_before"] = len(obj_prio)
    obj_prio = restrict_objecoes_structured(obj_prio, cf_id, title, MAX_OBJECOES_PRIORIZADAS)
    ...
    props_art = ensure_list(articulacao.get("proposicoes_nucleares"))
    counters["prop_art_before"] = len(props_art)
    props_art = restrict_proposicoes_structured(props_art, MAX_PROPOSICOES_ARTICULACAO)
    counters["prop_art_after"] = len(props_art)

    pontes_art = ensure_list(articulacao.get("pontes_entre_niveis"))
    counters["pontes_art_before"] = len(pontes_art)
    pontes_art = restrict_pontes_structured(pontes_art, MAX_PONTES)
    counters["pontes_art_after"] = len(pontes_art)

    anc_art = ensure_list(articulacao.get("ancoragens_cientificas"))
    counters["anc_art_before"] = len(anc_art)
    anc_art = restrict_ancoragens_structured(anc_art, MAX_ANCORAGENS)
    counters["anc_art_after"] = len(anc_art)

    campos_art = ensure_list(articulacao.get("campos_do_real"))
    counters["campos_art_before"] = len(campos_art)
    campos_art = restrict_campos_structured(campos_art, MAX_CAMPOS)
    counters["campos_art_after"] = len(campos_art)
    ...
    sequencia = truncate_list(unique_preserve_order_strings(sequencia), MAX_SEQUENCIA_REDACAO)
    ...
    checklist = truncate_list(unique_preserve_order_strings(checklist), MAX_CHECKLIST)
    counters["check_after"] = len(checklist)

    notas = [
        "Versão restringida com preservação de objetos estruturados.",
        ...
        "Subblocos de articulação estrutural foram mantidos como listas de objetos, sem achatamento em strings.",
    ]
    ...
    out["campo_ids_relacionados_restritos"] = campos_ids
    out["ponte_ids_relacionadas_restritas"] = pontes_ids
    out["ancoragem_ids_relacionadas_restritas"] = ancoragens_ids
    ...
    out["adjudicacao_filosofica_restrita"] = {
        ...
        "articulacao_estrutural_restrita": {
            "proposicoes_nucleares": props_art,
            "pontes_entre_niveis": pontes_art,
            "ancoragens_cientificas": anc_art,
            "campos_do_real": campos_art,
        },
        "decisao_de_adjudicacao_restrita": {
            ...
            "veredito_herdado_da_matriz": truncate_list(
                unique_preserve_order_strings(to_text_list(decisao.get("veredito_herdado_da_matriz"))),
                MAX_VEREDITO
            ),
        },
...
def transform(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "metadata": {
            ...
            "gerado_por_script": "restringir_adjudicacao_confrontos_filosoficos_v2.py",
            "descricao": (
                "Versão redacionalmente restringida da adjudicação dos confrontos filosóficos, "
                "preservando subestruturas como objetos/listas estruturadas."
            ),
        ...
        "fontes": {
            ...
            "finalidade": "Restrição redacional disciplinada da adjudicação filosófica sem achatamento estrutural.",
        },
        "estatisticas": {
            ...
            "campos_ids_antes": totals["campos_before"],
            "campos_ids_depois": totals["campos_after"],
            "pontes_ids_antes": totals["pontes_before"],
            "pontes_ids_depois": totals["pontes_after"],
            "ancoragens_ids_antes": totals["anc_before"],
            "ancoragens_ids_depois": totals["anc_after"],
            ...
            "proposicoes_articulacao_antes": totals["prop_art_before"],
            "proposicoes_articulacao_depois": totals["prop_art_after"],
            "pontes_articulacao_antes": totals["pontes_art_before"],
            "pontes_articulacao_depois": totals["pontes_art_after"],
            "ancoragens_articulacao_antes": totals["anc_art_before"],
            "ancoragens_articulacao_depois": totals["anc_art_after"],
            "campos_articulacao_antes": totals["campos_art_before"],
            "campos_articulacao_depois": totals["campos_art_after"],
        },
        "enums_documentados": {
            "estado_global": ["restringido"],
            "estado_restricao": ["restringido"],
        ...
        "regras_de_validacao": {
            "regras_gerais": [
                ...
                "Nenhuma lista restringida pode ultrapassar os máximos definidos.",
                "Objecoes priorizadas e articulação estrutural devem preservar estrutura em objetos/listas.",
            ]
        },
        "confrontos": out_confrontos,
        "observacoes_metodologicas": [
            "A restrição não substitui revisão filosófica humana.",
            "Esta versão foi desenhada para manter interoperabilidade entre scripts posteriores.",
        ],
    }
...
def validate_output(data: Dict[str, Any]) -> List[str]:
    ...
    for idx, item in enumerate(data["confrontos"], start=1):
        ...
        objr = adjr.get("objecoes_priorizadas_restritas")
        if not isinstance(objr, list):
            errors.append(f"{cf_id}: objecoes_priorizadas_restritas deve ser lista.")
        else:
            for j, obj in enumerate(objr, start=1):
                if not isinstance(obj, dict):
                    errors.append(f"{cf_id}: objecao priorizada #{j} não é objeto.")

        art = adjr.get("articulacao_estrutural_restrita")
        if not isinstance(art, dict):
            errors.append(f"{cf_id}: articulacao_estrutural_restrita deve ser objeto.")
        else:
            for key in ["proposicoes_nucleares", "pontes_entre_niveis", "ancoragens_cientificas", "campos_do_real"]:
                if key not in art or not isinstance(art[key], list):
                    errors.append(f"{cf_id}: articulacao_estrutural_restrita.{key} deve ser lista.")

            if len(ensure_list(art.get("proposicoes_nucleares"))) > MAX_PROPOSICOES_ARTICULACAO:
                errors.append(f"{cf_id}: articulacao_estrutural_restrita.proposicoes_nucleares excede máximo.")
...
def build_report(data: Dict[str, Any], validation_errors: List[str]) -> str:
    ...
    lines: List[str] = []
    lines.append("RELATÓRIO — RESTRIÇÃO DA ADJUDICAÇÃO DOS CONFRONTOS FILOSÓFICOS (V2)")
    lines.append("=" * 82)
    lines.append("")
    ...
    for item in data.get("confrontos", []):
        ...
        n_props = len(ensure_list(art.get("proposicoes_nucleares")))
        n_campos = len(ensure_list(art.get("campos_do_real")))
        n_pontes = len(ensure_list(art.get("pontes_entre_niveis")))
        n_anc = len(ensure_list(art.get("ancoragens_cientificas")))

        lines.append(f"* {cf_id} — {titulo}")
        lines.append(
            f"  teses={n_teses}; distinções={n_dist}; objeções={n_obj}; "
            f"proposições={n_props}; campos={n_campos}; pontes={n_pontes}; ancoragens={n_anc}"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Restringe redacionalmente a adjudicação dos confrontos filosóficos preservando estrutura."
    )
    parser.add_argument(
        "--project-root",
        dest="project_root",
