...
def get_fragment_text(fragment: Dict[str, Any]) -> str:
    for key in ('texto_integral', 'texto_fragmento', 'texto_curto', 'texto', 'fragmento', 'conteudo', 'text', 'body'):
        ...
        if isinstance(val, str) and val.strip():
            return val.strip()
    # fallback defensivo: tenta colher qualquer campo textual útil do fragmento
    harvested = []
    for k, v in fragment.items():
        if isinstance(v, str) and v.strip() and k not in {'id', 'origem_id', 'ficheiro_origem', 'tipo_unidade', 'funcao_textual_dominante', 'cadencia', 'estado_validacao', 'estado_excecao'}:
