...
DEFAULT_HINTS = [
    ...
    'sujeito', 'reflexividade', 'adequação', 'adequacao', 'verdade', 'erro', 'correção', 'correcao',
    'reinscrição no real', 'reinscricao no real', 'substancialismo', 'dissolução do sujeito', 'dissolucao do sujeito'
]

PROFILE_CONFIGS: Dict[str, Dict[str, Any]] = {
    'humano_situado_localidade_apreensao': {
        'label': 'Humano situado, localidade e apreensão',
        'description': 'Inscrição no real, ente localizado, apreensão situada e solo antropológico-local do problema.',
        'hints': [
            'humano situado', 'ser humano', 'ente localizado', 'localizado no real', 'localidade', 'localização', 'localizacao',
            'apreensão', 'apreensao', 'apreender', 'relação situada com o real', 'relacao situada com o real',
            'no real', 'inserido no real', 'situado no real', 'ponto de vista', 'relação com o real', 'relacao com o real',
            'nada apreende de fora do real', 'humano no real', 'humano encarnado'
        ],
        'boost_terms': ['human', 'local', 'apree', 'situad', 'real', 'ente'],
        'anti_hints': ['dissolução do sujeito', 'ficção discursiva', 'feixe de perceções']
    },
    'memoria_temporalidade_estabilizacao_interna': {
        'label': 'Memória, temporalidade e estabilização interna',
        'description': 'Solo temporal da retenção, continuidade interna, memória e estabilização do vivido.',
        'hints': [
            'memória', 'memoria', 'temporal', 'temporalidade', 'temporalidade interna', 'retenção', 'retencao',
            'estabilização interna', 'estabilizacao interna', 'continuidade interna', 'continuidade temporal',
            'identidade no tempo', 'identidade temporal', 'presença de si', 'presenca de si', 'retenção do vivido',
            'passado', 'retenção temporal', 'memória operativa', 'memória interna'
        ],
        'boost_terms': ['memor', 'tempor', 'reten', 'estab', 'conti', 'presen'],
        'anti_hints': ['erro de escala']
    },
    'representacao_mediacao_simbolo_linguagem': {
        'label': 'Representação, mediação, símbolo e linguagem',
        'description': 'Transformação da apreensão em representação operável e suas mediações simbólicas e linguísticas.',
        'hints': [
            'representação', 'representacao', 'representar', 'mediação', 'mediacao', 'símbolo', 'simbolo', 'linguagem',
            'signo', 'fixação', 'fixacao', 'operável', 'operavel', 'representação operável', 'representacao operavel',
            'mediação simbólica', 'mediacao simbolica', 'campo simbólico', 'campo simbolico', 'nomeação', 'nomeacao'
        ],
        'boost_terms': ['repre', 'media', 'simbo', 'lingu', 'sign', 'fixac'],
        'anti_hints': ['substância isolada']
    },
    'consciencia_reflexiva_sujeito_presenca_de_si': {
        'label': 'Consciência, reflexividade, sujeito e presença de si',
        'description': 'Camada reflexiva explícita do dossier: consciência, autoconsciência, presença de si e estatuto do sujeito.',
        'hints': [
            'consciência', 'consciencia', 'autoconsciência', 'autoconsciencia', 'consciência reflexiva', 'consciencia reflexiva',
            'reflexividade', 'reflexivo', 'sujeito', 'presença de si', 'presenca de si', 'autorrelação', 'autorrelacao',
            'estatuto do sujeito', 'si mesmo', 'autoconsciência reflexiva', 'subjetividade', 'subjetivo'
        ],
        'boost_terms': ['consc', 'refle', 'sujei', 'autoc', 'subje', 'autor'],
        'anti_hints': ['mero critério', 'mera adequação']
    },
    'adequacao_verdade_erro_correcao_reinscricao_no_real': {
        'label': 'Adequação, verdade, erro, correção e reinscrição no real',
        'description': 'Prolongamento epistemológico: adequação ao real, desadequação, verdade, erro e correção.',
        'hints': [
            'adequação', 'adequacao', 'desadequação', 'desadequacao', 'verdade', 'erro', 'correção', 'correcao',
            'reinscrição no real', 'reinscricao no real', 'submissão ao real', 'submissao ao real', 'critério', 'criterio',
            'ajustamento ao real', 'desajustamento', 'ajuste ao real', 'erro categorial', 'verdade sob critério'
        ],
        'boost_terms': ['adequa', 'verda', 'erro', 'corre', 'reins', 'crite'],
        'anti_hints': ['presença de si imediata']
    },
    'identidade_pessoal_substancialismo_dissolucao_do_sujeito': {
        'label': 'Identidade pessoal, substancialismo e dissolução do sujeito',
        'description': 'Perfil opcional para medir se o léxico clássico da subjetividade está a dominar artificialmente.',
        'hints': [
            'identidade pessoal', 'substancialismo', 'substância isolada', 'substancia isolada',
            'dissolução do sujeito', 'dissolucao do sujeito', 'ficção discursiva', 'ficcao discursiva',
            'feixe de perceções', 'feixe de percecoes', 'apercepção transcendental', 'apercepcao transcendental',
            'eu penso', 'eu substancial', 'polo de aparecimento', 'consciência pré reflexiva', 'consciencia pre reflexiva'
        ],
        'boost_terms': ['identi', 'subst', 'dissol', 'ficca', 'feixe', 'aperc'],
        'anti_hints': ['ente localizado', 'relação situada com o real']
    },
}
...
def build_overlap_signals(sample_global: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    corridor_like = (
        ...
        counts.get('consciencia_reflexiva_sujeito_presenca_de_si', 0)
    ) / total
    cf07_pull = (
        counts.get('representacao_mediacao_simbolo_linguagem', 0) +
        counts.get('adequacao_verdade_erro_correcao_reinscricao_no_real', 0)
    ) / total
    classic_subject_pull = counts.get('identidade_pessoal_substancialismo_dissolucao_do_sujeito', 0) / total

    notes: List[str] = []
    if cf03_like >= 0.45:
        notes.append('Há tração relevante para o solo CF03-like: humano situado/localidade/apreensão + memória.')
    if cf07_pull >= 0.40:
        notes.append('Há tração relevante para o corredor CF07-like: representação/mediação + adequação/verdade/erro/correção.')
    if corridor_like >= 0.60:
        notes.append('O sample aproxima-se fortemente do corredor P25–P30/P31: memória → representação → mediação → consciência reflexiva, com prolongamento epistemológico.')
    if classic_subject_pull >= 0.30:
        notes.append('O léxico clássico de sujeito/identidade/substancialismo tem peso não negligenciável no sample.')
    if not notes:
        notes.append('Os sinais de sobreposição arquitetónica existem, mas não aparecem de forma concentrada no sample selecionado.')

    return {
        'cf03_like_share': round(cf03_like, 4),
        'cf07_like_share': round(cf07_pull, 4),
        'corridor_p25_p31_share': round(corridor_like, 4),
        'classic_subject_share': round(classic_subject_pull, 4),
        'notes': notes,
    }
...
def build_markdown(
    ...
    lines: List[str] = []
    lines.append(f'# {confronto_id} — fragmentos relevantes para o dossier (CF04 v1)')
    lines.append('')
...
def build_report(
    ...
    lines: List[str] = []
    lines.append(f'RELATÓRIO — {confronto_id} (CF04 v1)')
    lines.append('')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seleciona fragmentos relevantes para o dossier CF04, com score global robusto, perfis paralelos, estimação do perfil declarado e sinais de sobreposição arquitetónica.'
    )
