...
def ensure_pipeline_dirs() -> None:
    for path in [INBOX_DIR, BASE_REF_DIR, INSTANCIA_DIR, RUNS_DIR, READY_DIR, ARCHIVE_DIR]:
...
def resolve_input_path(input_arg: str) -> Path:
    ...
    if raw.is_absolute():
        ...
        candidates = [Path.cwd() / raw, SCRIPT_DIR / raw, PIPELINE_DIR / raw, INBOX_DIR / raw]
...
def resolve_reference_path(ref_arg: str) -> Optional[Path]:
    raw = Path(ref_arg)
    if raw.is_absolute():
        candidates = [raw]
    else:
        candidates = [Path.cwd() / raw, SCRIPT_DIR / raw, PIPELINE_DIR / raw, BASE_REF_DIR / raw, INSTANCIA_DIR / raw, READY_DIR / raw]
    for candidate in candidates:
        if candidate.exists():
            return candidate.resolve()
    return None
...
def extract_max_f_from_text_file(path: Path) -> int:
    max_id = 0
    try:
...
def candidate_reference_paths(script_dir: Path) -> List[Path]:
    ...
    for pattern in ["**/fragmentos_resegmentados__*.json", "**/containers_segmentacao__*.json", "**/*.md"]:
...
def detect_start_index(
    ...
    max_found = 0
    refs = reference_paths or candidate_reference_paths(script_dir)
    for ref in refs:
        try:
            if ref.suffix.lower() == ".json":
                ...
                local_max = extract_max_f_from_text_file(ref)
            if local_max > 0:
...
def write_relatorio_txt(
    ...
    lines.append(f"- match={match_dir}")
    lines.append(f"- containers={containers_dir}")
    lines.append(f"- validacao={validation_dir}")
...
# -----------------------------------------------------------------------------
# Arquivo de inputs e match
# -----------------------------------------------------------------------------

def copy_if_exists(src: Path, dst_dir: Path, dst_name: Optional[str] = None) -> Optional[Path]:
    if not src.exists():
        return None
...
def archive_input_and_match(input_path: Path, input_dir: Path, match_dir: Path) -> None:
    copy_if_exists(input_path, input_dir, input_path.name)
    match_files = [
        INBOX_DIR / "fragmentos_novos.md",
        INBOX_DIR / "fragmentos_nao_processados__apenas_novos.md",
        INBOX_DIR / "fragmentos_nao_processados__possiveis_duplicados.md",
        INBOX_DIR / "relatorio_match_nao_processados_vs_processados.md",
        INBOX_DIR / "match_nao_processados_vs_processados.json",
    ]
    for src in match_files:
        copy_if_exists(src, match_dir)
...
def parse_args() -> argparse.Namespace:
    default_input = str(INBOX_DIR / "fragmentos_nao_processados__apenas_novos.md")
    parser = argparse.ArgumentParser(description="Gerar run de containers para fragmentos novos ainda não resegmentados.")
    parser.add_argument("--input", default=default_input, help="Ficheiro markdown de entrada. Por defeito usa 00_inbox_novos/fragmentos_nao_processados__apenas_novos.md.")
    parser.add_argument("--output-prefix", default="run_fragmentos_novos", help="Prefixo da run se --run-name não for indicado.")
    parser.add_argument("--run-name", default=None, help="Nome explícito da pasta da run. Se omitido, usa <output-prefix>__YYYYMMDD_HHMMSS.")
    parser.add_argument("--start-from", type=int, default=None, help="Forçar primeiro F#### da nova run, usando número sem F.")
    parser.add_argument("--reference-json", action="append", default=[], help="Ficheiro(s) JSON ou MD de referência para detetar o maior F####.")
...
def main() -> None:
    ...
    run_name = args.run_name or f"{args.output_prefix}__{ts}"
    run_dir = RUNS_DIR / run_name
    input_dir = run_dir / "00_input"
    match_dir = run_dir / "01_match"
    containers_dir = run_dir / "02_containers"
    reseg_dir = run_dir / "03_resegmentacao"
    validation_dir = run_dir / "04_validacao"
    reports_dir = run_dir / "05_relatorios"

    for d in [input_dir, match_dir, containers_dir, reseg_dir, validation_dir, reports_dir]:
        d.mkdir(parents=True, exist_ok=True)

    archive_input_and_match(input_path, input_dir, match_dir)
    output_md = containers_dir / f"{run_name}.md"
    output_json = containers_dir / f"containers_segmentacao__{run_name}.json"
    output_validation = validation_dir / f"relatorio_validacao_containers__{run_name}.json"
    output_txt = reports_dir / f"{run_name}__relatorio.txt"
    ...
    print("=" * 72)
    print("RUN DE CONTAINERS — FRAGMENTOS NOVOS (v4 pipeline)")
    print("=" * 72)
    ...
    print("-" * 72)
    print(f"Markdown run:        {output_md}")
    print(f"Containers JSON:     {output_json}")
    print(f"Validação JSON:      {output_validation}")
