...
Ajustes principais face às versões anteriores:
...
Saídas:
- fragmentos_resegmentados__<run>.json
- estado_resegmentador__<run>.json
- falhas_resegmentador__<run>.json
- relatorio_validacao_fragmentos__<run>.json
- relatorio_resegmentador__<run>.txt
"""
...
SCRIPT_VERSION = "resegmentador_semantico_v6_0_duas_fases_openai"
...
RE_SPLIT_SENTENCE = re.compile(r'(?<=[.!?])\s+(?=[A-ZÀ-Ý"“«(\[])', re.UNICODE)
...
def split_sentences(text: str) -> List[str]:
    ...
    # Junta fragmentos muito curtos à frase anterior
    merged: List[str] = []
    for part in parts:
...
def build_units(container: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], bool]:
    ...
    if len(paragraphs) == 1:
        ...
        # se o parágrafo for curto ou tiver poucas frases, mantém-se atomicamente
...
def _normalizar_schema_para_strict(schema: Any) -> Any:
    ...
    if "properties" in schema and isinstance(schema["properties"], dict):
        schema["properties"] = {k: _normalizar_schema_para_strict(v) for k, v in schema["properties"].items()}
    if "items" in schema:
...
def openai_generate_json(api: ApiConfig, prompt: str, schema_obj: Dict[str, Any], schema_name: str) -> Any:
    if CLIENT is None:
        raise RuntimeError("biblioteca_openai_indisponivel")
        ...
        "properties": {
            "grupos": {
                ...
                "items": {
                    ...
                    "properties": {
                        "indice": {"type": "integer"},
                        "tipo_unidade": {"type": "string", "enum": sorted(VALID_TIPO_UNIDADE)},
                        "criterio_de_unidade": {"type": "string", "enum": sorted(VALID_CRITERIO_UNIDADE)},
                        "funcao_textual_dominante": {"type": "string", "enum": sorted(VALID_FUNCAO)},
                        "tema_dominante_provisorio": {"type": "string"},
                        "conceitos_relevantes_provisorios": {
                            "type": "array",
                            "items": {"type": "string"},
                        },
                        "integridade_semantica_grau": {"type": "string", "enum": sorted(VALID_GRAU)},
                        "confianca_segmentacao": {"type": "string", "enum": sorted(VALID_CONFIANCA)},
...
def build_phase1_prompt(container: Dict[str, Any], units: List[Dict[str, Any]], internal_mode: bool) -> str:
    meta = {
        ...
        "n_chars_total": sum(int(p.get("n_chars", 0)) for p in container.get("paragrafos", [])),
...
def heuristic_plan(units: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def wants_merge(a: str, b: str) -> bool:
        ...
        if a_n.endswith(":"):
            return True
        starts = (
            "e ", "mas ", "ou ", "porque ", "portanto ", "logo ", "aliás ",
            "depois ", "assim ", "por isso ", "então ", "contudo ", "no entanto ",
        )
...
def build_phase2_prompt(container: Dict[str, Any], units: List[Dict[str, Any]], groups: List[Dict[str, List[str]]], internal_mode: bool) -> str:
...
def heuristic_annotation(fragment_text: str) -> Dict[str, Any]:
    ...
    conceitos = []
    candidates = [
        "real", "consciência", "consciencia", "representação", "representacao",
        "símbolo", "simbolo", "linguagem", "verdade", "erro", "correção",
        "correcao", "campo", "potencialidades", "liberdade", "dignidade",
        "sistema", "descrição", "descricao", "apreensão", "apreensao",
    ]
    for c in candidates:
        if c in lower:
            v = c.replace("consciencia", "consciência").replace("representacao", "representação") \
                 .replace("simbolo", "símbolo").replace("correcao", "correção") \
                 .replace("descricao", "descrição").replace("apreensao", "apreensão")
...
def normalize_annotation(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    func_map = {"afirmação": "afirmacao", "crítica": "critica", "distinção": "distincao", "síntese": "sintese"}
    ...
    out["conceitos_relevantes_provisorios"] = coerce_list_str(out.get("conceitos_relevantes_provisorios"), max_items=5)
    ...
    out["requer_revisao_manual_prioritaria"] = bool(out.get("requer_revisao_manual_prioritaria", False))
    return out
...
def annotate_groups(container: Dict[str, Any], units: List[Dict[str, Any]], groups: List[Dict[str, List[str]]], api: ApiConfig, use_api: bool, internal_mode: bool) -> List[Dict[str, Any]]:
    unit_map = {u["unit_id"]: u for u in units}
    by_idx: Dict[int, Dict[str, Any]] = {}

    if use_api:
        raw = openai_generate_json(api, build_phase2_prompt(container, units, groups, internal_mode), phase2_schema(), "phase2_segment_annotations")
    ...
    for i, group in enumerate(groups, start=1):
        if i in by_idx:
            ...
            ann = normalize_annotation(heuristic_annotation(build_group_text(unit_map, group["unidades_origem"])))
        out.append(ann)
    return out


def build_fragment(container: Dict[str, Any], units: List[Dict[str, Any]], group: Dict[str, List[str]], ann: Dict[str, Any], idx: int, model_name: str, internal_mode: bool) -> Dict[str, Any]:
    unit_map = {u["unit_id"]: u for u in units}
    ids = group["unidades_origem"]
    ...
    return {
        ...
        "integridade_semantica": {"grau": ann["integridade_semantica_grau"]},
...
def validate_output(fragments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for frag in fragments:
        ...
        if fid in ids:
            errors.append({"fragment_id": str(fid), "codigo": "fragment_id_duplicado", "detalhe": str(fid)})
        ids.add(fid)

        tipo = frag.get("segmentacao", {}).get("tipo_unidade")
        if tipo not in VALID_TIPO_UNIDADE:
            errors.append({"fragment_id": str(fid), "codigo": "tipo_unidade_invalido", "detalhe": f"tipo_unidade inválido: {tipo}"})

        criterio = frag.get("segmentacao", {}).get("criterio_de_unidade")
        if criterio not in VALID_CRITERIO_UNIDADE:
            errors.append({"fragment_id": str(fid), "codigo": "criterio_unidade_invalido", "detalhe": f"criterio_de_unidade inválido: {criterio}"})

        func = frag.get("funcao_textual_dominante")
        if func not in VALID_FUNCAO:
            errors.append({"fragment_id": str(fid), "codigo": "funcao_textual_invalida", "detalhe": f"funcao_textual_dominante inválida: {func}"})
...
def process_container(container: Dict[str, Any], api: ApiConfig, use_api: bool) -> Tuple[List[Dict[str, Any]], Dict[str, Any], List[Dict[str, Any]]]:
    state = {
        "status": "ok",
        "versao_segmentador": SCRIPT_VERSION,
        "ts": utc_ts(),
    ...
    try:
        ...
        if len(units) == 1:
            ...
            if use_api:
                plan_obj = openai_generate_json(api, build_phase1_prompt(container, units, internal_mode), phase1_schema(), "phase1_segment_plan")
        ...
        anns = annotate_groups(container, units, groups, api, use_api, internal_mode)
        frags = [
            build_fragment(container, units, grp, ann, i, api.model if use_api else "heuristico_local", internal_mode)
...
def build_txt_report(run_name: str, input_path: Path, output_json: Path, validation: Dict[str, Any], states: Dict[str, Any], failures: List[Dict[str, Any]]) -> str:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ressegmentador semântico em duas fases para containers estabilizados.")
    parser.add_argument("input_json", help="JSON de containers de entrada")
    parser.add_argument("--output-dir", default=".", help="Pasta de saída")
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Modelo OpenAI")
    parser.add_argument("--local-only", action="store_true", help="Força modo heurístico local, sem API")
    ...
    output_json = out_dir / f"fragmentos_resegmentados__{run_name}.json"
    state_json = out_dir / f"estado_resegmentador__{run_name}.json"
    failures_json = out_dir / f"falhas_resegmentador__{run_name}.json"
    validation_json = out_dir / f"relatorio_validacao_fragmentos__{run_name}.json"
    report_txt = out_dir / f"relatorio_resegmentador__{run_name}.txt"

    api = ApiConfig(model=args.model, retries=max(1, args.retries))
    use_api = bool(os.getenv("OPENAI_API_KEY")) and not args.local_only
    ...
    for container in containers:
        frags, state, fail = process_container(container, api, use_api)
        if frags:
    ...
    write_json(state_json, {"versao_segmentador": SCRIPT_VERSION, "containers": states})
    write_json(failures_json, failures)
    write_json(validation_json, validation)
    report_txt.write_text(build_txt_report(run_name, input_path, output_json, validation, states, failures), encoding="utf-8")
